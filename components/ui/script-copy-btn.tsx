...
export function ScriptCopyBtn({
  ...
  useEffect(() => {
    async function loadTheme(themeNameOrObject: string | object) {
      if (typeof themeNameOrObject === "string") {
        // Try to fetch external theme JSON if not a built-in theme
        const builtInThemes = [
          "github-light",
          "github-dark",
          "min-light",
        ...
        if (builtInThemes.includes(themeNameOrObject)) return themeNameOrObject;
        try {
          const res = await fetch(
            `https://unpkg.com/shiki/themes/${themeNameOrObject}.json`
          );
          if (res.ok) return await res.json();
        } catch {}
        return themeNameOrObject; // fallback
      }
    ...
    async function loadHighlightedCode() {
      try {
        ...
        const loadedLightTheme = await loadTheme(lightTheme);
        const loadedDarkTheme = await loadTheme(darkTheme);
